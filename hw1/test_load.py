...
class DataGenerator(object):
    ...
    def sample_batch(self, batch_type, batch_size, shuffle=True, swap=False):
        ...
        for i in range(batch_size):
            ...
            labels = np.reshape(
                labels, (self.num_classes, self.num_samples_per_class))
            labels = np.eye(self.num_classes)[labels]

            ims = np.reshape(
                ims, (self.num_classes, self.num_samples_per_class, -1))
...
imgs, labels = generator.sample_batch('train', 5, swap=True)
