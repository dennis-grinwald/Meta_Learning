...
class Model(object) :
    ...
    def __init__(self, num_act, scope, reuse) :
        ...
        with tf.variable_scope(scope,reuse = reuse):
            ...
            self.inp = tf.placeholder(shape = [None, NUM_DIM*2],dtype = tf.float32)
...
def update_replay_buffer(episode_experience, HER):
    ...
    for t in range(STEPS_PER_EPISODE) :
        ...
        if HER == 'None':
            ...
            new_goal = episode_experience[-1][3][:NUM_DIM]
            ...
            random_int = np.random.choice(np.random.permutation(STEPS_PER_EPISODE - t))
            new_goal = episode_experience[-random_int][3][:NUM_DIM]
            ...
            random_int = np.random.choice(np.random.permutation(NUM_DIM))
            new_goal = episode_experience[-random_int][3][:NUM_DIM]
